...
class SocialiteGuard implements Guard
{
    ...
    private function checkUser()//ia user-ul si verifica daca e logat sau nu
    {
        if($this->fetched_user === false)
        {
            ...
            $my_user = User::where('oauth_id', $oauthUser->id)->first();
            if($my_user !== null)
            {
                $this->user = $my_user;
            }
...
//            }
            ...
            $this->fetched_user = true;//seteaza fetch_id true indiferent de actiunea pe care o exercita pt a nu verifica user-ul la infinit
        }
    }
